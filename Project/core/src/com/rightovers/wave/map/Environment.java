...
public class Environment implements IResourceable {
    ...
    public void drawBackground(float deltaTime) {
        ...
        drawTextureSequence(this.background, this.backgroundPositions, Player.getInstance().getDistance() / 2 - lastDistance / 2);
        drawTextureSequence(this.ground, this.groundPositions, Player.getInstance().getDistance() - lastDistance);

        lastDistance = Player.getInstance().getDistance();
